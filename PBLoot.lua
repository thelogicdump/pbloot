...
function CalculateDKPValue(ilvl, slot_mod)
    if ilvl == 264 then
        return Round(0.483 * (2^(ilvl/28)) * slot_mod, 0)
    else
...
function dkp_string(ilvl, slot)
    ...
    elseif slot == "INVTYPE_NECK" then
        if ilvl >= 264 then
            slotmod = 0.75
        else
            slotmod = 0.5
        end
        return BuildDKPString("Neck", ilvl, slotmod)
    elseif slot == "INVTYPE_SHOULDER" then
        return BuildDKPString("Shoulder", ilvl, 0.75)
    elseif slot == "INVTYPE_CHEST" then
        return BuildDKPString("Chest", ilvl, 1)
    elseif slot == "INVTYPE_ROBE" then
        return BuildDKPString("Chest", ilvl, 1)
    elseif slot == "INVTYPE_WAIST" then
        return BuildDKPString("Waist", ilvl, 0.75)
    elseif slot == "INVTYPE_LEGS" then
        return BuildDKPString("Legs", ilvl, 1)
    elseif slot == "INVTYPE_FEET" then
        return BuildDKPString("Feet", ilvl, 0.75)
    elseif slot == "INVTYPE_WRIST" then
        return BuildDKPString("Wrist", ilvl, 0.5)
    elseif slot == "INVTYPE_HAND" then
    ...
    elseif slot == "INVTYPE_CLOAK" then
        ...
        return BuildDKPString("Cloak", ilvl, slotmod)
    elseif slot == "INVTYPE_FINGER" then
        if ilvl >= 264 then
            slotmod = 0.75
        else
            slotmod = 0.5
        end
        return BuildDKPString("Finger", ilvl, slotmod)
    elseif slot == "INVTYPE_TRINKET" then
        if ilvl >= 264 then
            slotmod = 1
        else
            slotmod = 0.75
        end
        return BuildDKPString("Trinket", ilvl, slotmod)
    elseif slot == "INVTYPE_WEAPON" then
        ...
        return cost_string
    elseif slot == "INVTYPE_SHIELD" then
